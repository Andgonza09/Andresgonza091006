...
namespace Ejercicio_8
{
    internal class Program
    {
        static void Sistema_Tickets(int cantidad_tickets, int min, int max)
        {
            ...
            // Creamos un nuevo array para mostrar los números generados con el mismo tamaño que el anterior para evitar errores
            int[] nuevo_arr = new int[Número_Tickets];
            for (int i = 0; i < Número_Tickets; i++)
            {
                // Usamos el método Next de la clase Random para generar un número aleatorio dentro del rango indicado
                Cantidad[i] = random.Next(rango_min, rango_max);

                // Asignamos los valores generados al nuevo array, es decir que los elementos del array Cantidad se copian en el array nuevo_arr
                nuevo_arr[i] = Cantidad[i];
            }
